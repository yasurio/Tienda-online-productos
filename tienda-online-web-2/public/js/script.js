...
function init() {
    ...
    function loadData() {
        ...
        let url = "http://localhost:8080/tienda/producto/findAll";
    ...
    function refreshItemsCart() {
        ...
        for (let i = 0; i < currentCart.length; i++) {
            ...
            hijo.innerHTML = `
                <a href="shopping-cart.html" title="Producto">${currentCart[i].descripcion}</a><span class="at-shopping-cart-price">${currentCart[i].precio}€</span>
            ...
            totalPrecio += currentCart[i].precio;
        }
    ...
    function removeArticle(element, index) {
        ...
        totalPrecio = totalPrecio - currentCart[index].precio;
        document.querySelector('#totalPrice').textContent = totalPrecio;
    ...
    function filter() {
        ...
        result = data.filter(article => {
            return article.descripcion.toLowerCase().indexOf(this.value.toLowerCase()) != -1;
        });
        loadArticles(result);
    }
